...
namespace Protostar
{
    public abstract class BaseProject : Project
    {
        ...
        private void SetConfiguration(Configuration configuration, Target target)
        {
            ...
            configuration.IncludePaths.Add(Path.Combine("[project.SharpmakeCsPath]", "Source", "Private"));
            ...
            configuration.TargetPath = Path.Combine(MainSolution.GeneralOutputPathFolder, target.Optimization.ToString());
    ...
    public abstract class Module : BaseProject
    {
        ...
        public override void ConfigureAll(Configuration configuration, Target target)
        {
            ...
            configuration.Defines.Add($"{GetType().Name.ToUpper()}_LIBRARY");
    ...
    public class MainSolution : Solution
    {
        ...
        {
            RootFolder = "[solution.SharpmakeCsPath]";
            ...
            configuration.SolutionPath = "[solution.SharpmakeCsPath]";
            configuration.SolutionFileName = "[solution.Name]";
            Type[] _moduleTypes = Assembly.GetAssembly(typeof(BaseProject)).GetTypes();
            HashSet<Type> _executingProjects = new HashSet<Type>();
            foreach (Type _type in _moduleTypes)
            {
                if(!_type.IsClass || _type.IsAbstract)
                {
                    continue;
                }
                ...
                else if (_type.IsSubclassOf(typeof(ExecutingProject)))
                {
