...
#pragma warning( suppress : 4127 )
    ...
    HRESULT LoadVBO(_In_z_ const wchar_t* szFileName)
    {
        ...
        vboFile.read(reinterpret_cast<char*>(vertices.data()), sizeof(Protostar::Vertex) * numVertices);
...
#pragma warning( suppress : 4127 )
        ...
        BoundingBox::CreateFromPoints(bounds, vertices.size(), reinterpret_cast<const XMFLOAT3*>(vertices.data()), sizeof(Protostar::Vertex));
    ...
    std::vector<Protostar::Vertex>     vertices;
...
private:
    ...
    uint32_t AddVertex(uint32_t hash, const Protostar::Vertex* pVertex, VertexCache& cache)
    {
        ...
        for (auto it = f.first; it != f.second; ++it)
        {
            ...
            if (0 == memcmp(pVertex, &tv, sizeof(Protostar::Vertex)))
            {
                return it->second;
            }
        }
